...
export default function PDFToPNGComponent() {
  ...
  const handleConvertToPng = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= count; i++) {
        setProgressMsg(`Đang render trang ${i}/${count} thành PNG...`);
        ...
        zip.file(`${cleanName}-trang-${i}.png`, base64Data, { base64: true });
      }
  ...
  const handleDownload = () => {
    ...
    link.download = `${cleanName}-images-png.zip`;
    link.click();
  };
              ...
              {zipBlobUrl && (
                <div className="w-full space-y-4">
                  <div className="p-3.5 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-250 dark:border-emerald-850 rounded-xl text-center text-xs font-bold text-emerald-600 dark:text-emerald-400">
                    ✓ Đã trích xuất {totalPages} ảnh PNG thành công!
                  </div>
                  <button
