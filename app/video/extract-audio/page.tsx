...
export default function ExtractAudioPage() {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      // Extract MP3: ffmpeg -i input.mp4 -q:a 0 -map a output.mp3
      await ffmpeg.exec([
        "-i",
        inputName,
        "-q:a",
      ...
      const baseName = videoFile.name.substring(0, videoFile.name.lastIndexOf(".")) || "extracted";
      ...
      console.error(err);
      alert("Đã xảy ra lỗi trong quá trình tách nhạc.");
    } finally {
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <span className="text-xs bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-455 px-3 py-1 rounded-full border border-emerald-500/20 font-medium">
            🔒 Bảo mật tuyệt đối - Mọi trích xuất âm nhạc diễn ra ngay tại thiết bị của bạn
          </span>
        </div>
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2">
            🎵 Tách nhạc từ video sang MP3
          </h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Trích xuất lấy riêng tệp âm thanh định dạng MP3 chất lượng cao từ các video clip của bạn hoàn toàn offline trên trình duyệt.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Uploader (col: 6) */}
          <div className="lg:col-span-6 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-755 shadow-md p-6 flex flex-col items-center justify-center min-h-[300px] relative">
            {!videoFile ? (
              <div className="flex flex-col items-center justify-center text-center p-8 border-3 border-dashed border-gray-250 dark:border-gray-700 rounded-2xl w-full hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-colors relative cursor-pointer min-h-[240px]">
                <input
                  type="file"
                  accept="video/*"
              ...
              <div className="w-full space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {isProcessing ? "Đang trích xuất..." : "📥 Tải nhạc MP3"}
                  </button>
                </div>
              </div>
            )}
          ...
          {/* Guidelines (col: 6) */}
          <div className="lg:col-span-6 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-755 shadow-md p-6 space-y-4">
            <h3 className="font-bold text-sm text-gray-855 dark:text-gray-250 border-b border-gray-100 dark:border-gray-750 pb-2">
              Ưu điểm nổi bật
            </h3>

            <ul className="space-y-3 text-xs text-gray-600 dark:text-gray-300 font-medium">
              <li className="flex items-start gap-2">
                <span className="text-emerald-500">✓</span>
                <span><strong>Bảo mật 100%:</strong> Video không bao giờ tải lên bất kỳ máy chủ nào. Trích xuất âm thanh ngay tại bộ nhớ trình duyệt máy tính của bạn.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-emerald-500">✓</span>
                <span><strong>Chất lượng cao:</strong> Chuyển hóa kênh tiếng trong video thành tệp MP3 với độ nén tối ưu, giữ nguyên chất giọng trong sạch nhất.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-emerald-500">✓</span>
                <span><strong>Không giới hạn số lần:</strong> Hoàn toàn miễn phí, không chèn watermark quảng cáo, không ép đăng ký.</span>
              </li>
            </ul>
          </div>
        </div>
