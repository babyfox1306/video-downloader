...
export default function ExcelToCSVPage() {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        try {
          ...
          // Get preview of the first sheet
          ...
          console.error(err);
          alert("Không thể đọc tệp Excel này. Vui lòng kiểm tra lại định dạng tệp.");
        } finally {
  ...
  const handleConvert = () => {
    ...
    try {
      ...
      console.error(err);
      alert("Đã xảy ra lỗi trong quá trình chuyển đổi Excel sang CSV.");
    }
  };
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <span className="text-xs bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-455 px-3 py-1 rounded-full border border-emerald-500/20 font-medium">
            🔒 Xử lý offline - Tập tin của bạn tuyệt đối không rời máy tính
          </span>
        </div>
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Tải lên tệp Excel (.xlsx, .xls) của bạn để trích xuất nhanh dữ liệu của trang tính đầu tiên thành tệp CSV mỏng nhẹ hơn.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-755 shadow-md p-6">
            <h3 className="font-bold text-sm text-gray-850 dark:text-gray-200 border-b border-gray-100 dark:border-gray-750 pb-2 mb-4">
              Xem trước 10 dòng đầu tiên (Sheet 1)
            </h3>
            ...
            {previewData.length > 0 ? (
              ...
              <div className="text-center py-16 text-gray-400 text-xs italic">
                Chưa có dữ liệu bảng tính. Tải tệp Excel bên trái để xem trước bảng biểu.
              </div>
            )}
          </div>
        </div>
